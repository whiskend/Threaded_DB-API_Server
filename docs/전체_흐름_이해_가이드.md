...
flowchart LR
    ...
    subgraph Server["mini_db_server 내부 흐름"]
        ...
        subgraph DBAPI["6. db_api.c가 감싸는 엔진 진입 흐름"]
            ...
            Parser["6-2. parser.c<br/>parse_statement"]
            Policy["6-3. db_api.c<br/>단일 SQL 검증 + preload/lock"]
...
5. `db_api.c`가 `lexer -> parser`를 호출해서 AST를 한 번 만든다.
6. `db_api.c`가 그 같은 AST를 보고 단일 SQL 검증, preload, 락 정책을 적용한다.
7. `db_api.c`가 그 같은 AST를 `executor`로 넘긴다.
8. `executor`는 필요하면 runtime/schema/storage/index 계층을 내려가며 실제 작업을 수행한다.
9. 결과를 JSON으로 바꿔 HTTP 응답으로 돌려준다.
...
flowchart LR
    ...
    subgraph EntryFlow["6. db_api.c가 직접 묶는 호출 흐름"]
        ...
        D3["6-2. parse_statement<br/>src/parser.c"]
        D4["6-3. 단일 SQL 검증 + preload/lock<br/>src/db_api.c"]
...
4. 진짜 DB 파일을 직접 읽고 쓰는 것은 새 서버 파일이 아니라 기존 엔진의 `runtime.c`, `storage.c`, `schema.c`, `bptree.c` 쪽이다.
...
flowchart TD
    ...
    subgraph H["db_api.c 내부 호출 흐름"]
        H1["lexer.c: tokenize_sql"]
        H2["parser.c: parse_statement"]
        H3["db_api.c: 단일 SQL 검증 + preload/lock"]
        H4["executor.c: execute_statement"]
    end
